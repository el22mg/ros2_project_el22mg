...
class colourIdentifier(Node):
    def __init__(self):
        ...
        # Define publishing of detection of green object
        self.publisher = self.create_publisher(String, 'greenObjectDetected', 10)
        

    def callback(self, data):

        # Convert the received image into a opencv image
        # (But remember to wrap calls to this conversion method in an exception handler)
        try:
        ...
        greenMaskedImage = cv2.bitwise_and(Hsv_image, Hsv_image, mask=greenMask)

        # Find the contours that appear within the certain colour mask using the cv2.findContours() method
        # For <mode> use cv2.RETR_LIST for <method> use cv2.CHAIN_APPROX_SIMPLE
        contours, _ = cv2.findContours(greenMask, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE) # Use mask, not image itself, as contour function expects binary (i.e. foreground-background) image input 
        ...
        if self.greenDetected:
            msg = String()
            msg.data = f'Green object detected :P'
        ...
        # Green-masked image
        cv2.namedWindow('gMasked_camera_Feed',cv2.WINDOW_NORMAL)
        cv2.imshow('gMasked_camera_Feed', greenMaskedImage)
        cv2.resizeWindow('gMasked_camera_Feed',320,240)
        cv2.waitKey(3)
        
        # Circled Green Objects image
        cv2.namedWindow('circledGreenObjects_camera_Feed',cv2.WINDOW_NORMAL)
        cv2.imshow('circledGreenObjects_camera_Feed', image)
        cv2.resizeWindow('circledGreenObjects_camera_Feed',320,240)
        cv2.waitKey(3)
